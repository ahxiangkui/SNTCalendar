 ...
 */
public class DateInWeekendChoiceRVAdapter extends RecyclerView.Adapter<DateInWeekendChoiceRVAdapter.MViewHolder> {
    ...
    public DateInWeekendChoiceRVAdapter(Context context, Calendar selectDate, Calendar minDate, Calendar maxDate
            , int tintColor, int tintAlpha, RVItemEventListener rvItemEventListener, DateItemThemeSetupCallback themeSetupCallback, int itemLayout, List<Integer> enableDays) {
    ...
    private void onBindNormalView(MViewHolder holder, int position){
        ...
        if (holder instanceof NormalViewHolder){
            ...
            if (selectDate != null && dateEquals(selectDate, item) == 0){
                ...
                if (enableDays != null && enableDays.size() > 0){

                    if (enableDays.contains(item.get(Calendar.DAY_OF_WEEK))){
                        ((NormalViewHolder) holder).disable = false;
                    }else {
                        ((NormalViewHolder) holder).disable = true;
                    }
